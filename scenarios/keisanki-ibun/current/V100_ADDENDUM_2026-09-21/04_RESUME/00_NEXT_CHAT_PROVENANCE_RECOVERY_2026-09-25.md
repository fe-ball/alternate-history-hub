...
Updated: 2026-09-25, after explicit user audit/reset instruction and Shiun/Oyodo development audit
...
先に [紫雲・大淀の開発史／艦上計算機監査03](AUDIT_RESET_2026-09-25/03_SHIUN_OYODO_AUDIT_WORKING_v001.md)、[共通生産・索敵入力の整理04](AUDIT_RESET_2026-09-25/04_SHARED_PRODUCTION_AND_RECON_INTERFACES.md) を読む。

保持資料30には比叡→長門→大和→武蔵の実機艦載計算系列がある。大淀だけ計算機を持たない前提へ黙って戻さない。ただし、大淀の実搭載記録を発見したわけではない。03の接触処理装置、概念設計枠、1941–43試験／艤装経路は新しいWORKING案で、実艤装・性能・量産・6月所在は別に閉じる。

紫雲は二重反転、補助浮舟引込、主浮舟投棄を別採否として比較する。強風・火星・川西水上航空の成熟を継承するが、一律性能加算や史実失敗の自動継承はしない。JACAR原画像未取得の飛行試験／実用部隊事項を確認済みにしない。03は新たなE15K最高速度・生産数・6/18即応数を確定していない。
...
- 紫雲は「少し良い判断でほどほどに改善」に丸めない。個々の実測とBranch設計・試験能力から判断し、旧18日午後の結果を維持するための値調整はしない。
